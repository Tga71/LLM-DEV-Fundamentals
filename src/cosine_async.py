import os, re, json, openai, time, math, asyncio
...
def llm_error_handler(query_llm):
    async def wrapper(*args, **kwargs):
        ...
        while True:
            try:
                result = await query_llm(*args, **kwargs)
                return result
...
@llm_error_handler
async def query_llm_completion(llm_client, messages):
...
@llm_error_handler
async def query_llm_embedding(llm_client, input_data):
...
async def process_input_data(llm_client, messages, consolidated_data):
    llm_output = await query_llm_completion(llm_client, messages)
...
async def print_similar_situations(llm_client, problem, situations_data):
    problem_embedding = await query_llm_embedding(llm_client, problem)

    situations_to_be_sorted = []
    for situation in situations_data.keys():
        situation_embedding = await query_llm_embedding(llm_client, situation)
...
async def main():
    ...
    async with asyncio.TaskGroup() as tg:
        for title, content in sections:
            ...
            tg.create_task(
                process_input_data(llm_client, messages, consolidated_data)
            )

    llm_client = openai.OpenAI(
        api_key=os.getenv("INFORMATOM"),
    )

    async with asyncio.TaskGroup() as tg:
        tg.create_task(
            print_similar_situations(
                llm_client,
